...
import { Lock, AlertTriangle, RefreshCw, WifiOff, AlertCircle } from 'lucide-react';

/**
 * PayrollErrorStates.jsx
 * Precision Enterprise error displays for EWMP Payroll Management.
 */
  ...
  return (
    <div className="bg-[#ffffff] dark:bg-[#111111] border border-rose-200 dark:border-rose-900/60 rounded-lg p-8 text-center max-w-md mx-auto my-8 shadow-xs">
      ...
      <p className="text-xs text-[#737686] dark:text-gray-400 mb-6 leading-relaxed">
        Your current role does not have authorization to execute payroll runs, modify statutory salary structures, or disburse bank settlements. Contact your Finance Controller or Super Administrator.
      </p>
      {onBack && (
        <button
          onClick={onBack}
...
export const NetworkErrorState = ({ onRetry }) => {
  return (
    <div className="bg-[#ffffff] dark:bg-[#111111] border border-amber-200 dark:border-amber-900/60 rounded-lg p-8 text-center max-w-md mx-auto my-8 shadow-xs">
      ...
      <h3 className="text-base font-bold text-[#191b23] dark:text-white mb-1.5">Gateway Communication Error</h3>
      <p className="text-xs text-[#737686] dark:text-gray-400 mb-6">
        We encountered a temporary timeout while communicating with the Bank Disbursal or Tax API server. Please verify network connectivity and retry.
      </p>
      <button
...
export const NoInternetBanner = () => {
  return (
    <div className="bg-slate-900 text-white px-4 py-2 text-xs flex items-center justify-center gap-2 font-mono">
      ...
      <span>Offline Mode — Payroll calculation runs and bank settlement uploads are paused until internet connection is restored.</span>
    </div>
  );
};
